...
import { deleteProductCategoryForm, upsertProductCategory } from "@/actions/admin";
import {
...
export function ProductCategoryEditorForm({ category }: Props) {
  ...
  return (
    <div className="max-w-4xl">
      <div className="mb-6">
        <Link href="/admin/product-categories" className={`${adminMutedLink} inline-flex items-center gap-1.5`}>
        ...
        {!isEdit ? (
          <p className="mt-2 max-w-xl text-sm leading-relaxed text-neutral-600">
            Nama tampil di kartu produk; slug untuk URL internal (unik). Kosongkan slug untuk dibuat dari nama.
          </p>
        ) : null}
      </div>

      <AdminSaveForm
        action={upsertProductCategory}
        className="space-y-8"
      ...
      >
        ...
        <AdminFormSection
          title="Kategori"
          description="Urutan menentukan tampilan di dropdown saat mengedit produk."
        >
          <div>
            <label htmlFor="pc-name" className={adminLabel}>
              Nama
            </label>
            <input id="pc-name" name="name" required defaultValue={c?.name} className={adminInput} />
          </div>
          <div>
            <label htmlFor="pc-slug" className={adminLabel}>
          ...
          <div className="max-w-xs">
            <label htmlFor="pc-sort" className={adminLabel}>
              Urutan
            </label>
            <input
              id="pc-sort"
        ...
        <div className="flex flex-col gap-3 border-t border-neutral-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
          <Link href="/admin/product-categories" className={`${adminGhostLink} py-2 sm:py-0`}>
            Batal
          </Link>
      ...
      {isEdit && c ? (
        <aside className="mt-10 rounded-2xl border border-red-200/80 bg-red-50/40 p-5 sm:p-6">
          ...
          <p className="mt-1 text-sm text-red-800/90">
            Hanya bisa dihapus jika tidak ada produk yang memakai kategori ini.
          </p>
          <form action={deleteProductCategoryForm} className="mt-4">
